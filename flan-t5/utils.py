...
def clean_text(texts: List[Union[str, None]], labels: List[Union[str, None]]) -> pd.DataFrame:
    """
...
def get_newsgroup_data(args, tokenizer: AutoTokenizer) -> List[Union[DatasetDict, int, int]]:
    ...
    max_source_length = max(
        [len(x) for x in tokenized_inputs["input_ids"]]
    )
    ...
    max_target_length = max(
        [len(x) for x in tokenized_targets["input_ids"]]
    )
